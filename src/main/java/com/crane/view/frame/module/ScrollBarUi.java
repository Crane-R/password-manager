...
//                trackBounds.height - 1);
    ...
    protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
        ...
        // 设置把手颜色
        g.setColor(Color.decode(Constant.colorConfig.get("handle")));
        // 画一个圆角矩形
        // 这里面前四个参数就不多讲了，坐标和宽高
...
//        return button;
        JButton button = new JButton("^");
        button.setBackground(Color.decode(Constant.colorConfig.get("upBtn")));
    ...
    protected JButton createIncreaseButton(int orientation) {
        ...
        button.setBackground(Color.decode(Constant.colorConfig.get("downBtn")));
        button.setFocusPainted(false);
        button.setBorderPainted(false);
        button.setBorder(null);
        return button;
